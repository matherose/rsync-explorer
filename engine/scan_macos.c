/**
 * @file scan_macos.c
 * @brief Local directory scanning on macOS using opendir/readdir/lstat.
 *
 * Uses the same POSIX approach as scan_posix.c since getattrlistbulk
 * requires complex buffer layout and doesn't expose file mode bits.
 * Performance is excellent for single-directory scans (our use case).
 * getattrlistbulk optimization can be added later if needed.
 */
...
int scan_dir_macos(const char *abs_path, file_entry_array_t *out)
{
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        fe.mtime = (int64_t)st.st_mtimespec.tv_sec;  /* macOS: st_mtimespec */
